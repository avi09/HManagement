...
ResultSet rs = ms.getData("select count(*) from outpatient where id = "+Integer.parseInt(id)+";");
...
try
{
	...
	{
		response.getWriter().print("<html><body><form action='outpatientlogin.jsp' id = 'sb'></form>"
	...
	rs = ms.getData("select * from outpatient where id = " + id + ";");
	rs.next();
	int count = 0;
	ArrayList<String> iter = new ArrayList<String>(Arrays.asList("id", "name", "age", "address", "visits", "docname", "illness", "contact"));
	...
	rs = ms.getData("select * from labtests where ref_id = " + id + " and category = 'out';");
	count = 0;
	...
	rs = ms.getData("select * from bills where ref_id = " + id + " and category = 'out';");
	count = 0;
